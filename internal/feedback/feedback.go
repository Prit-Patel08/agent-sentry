...
func GenerateFeedback(data FeedbackData) error {
	...
	sb.WriteString("# 🛡️ FlowForge Feedback\n\n")
	sb.WriteString("---\n\n")
	sb.WriteString("## ⚠️ Loop Detected in Your Process\n\n")
	sb.WriteString("FlowForge detected that your process entered a **repetitive loop** and intervened.\n\n")
	...
	sb.WriteString("1. **Check for infinite loops** — Review any `while True` or recursive calls in your code.\n")
	sb.WriteString("2. **Add exit conditions** — Ensure your loops have proper termination criteria.\n")
	sb.WriteString("3. **Rate-limit API calls** — If calling external APIs in a loop, add delays.\n")
	sb.WriteString("4. **Check for stale state** — Ensure variables are being updated within the loop body.\n\n")
...
func CleanupFeedback(path string) {
	os.Remove(path)
	fmt.Printf("[FlowForge] 🗑️  Feedback file cleaned up: %s\n", path)
}
